...
bayesiannetwork::bayesiannetwork(char* train_file, char* test_file, char* cfg_file)
{
	...
	for(int f2=0 ; f2< attributes ; f2++)
	{
		for(int f3=(f2*numclass[attributes]) ; f3 < ( (f2+1)*numclass[attributes] ) ; f3++)
		{
			...
			{
				ccc[f3][f4] = 0;
				//f3表示第f3个ccc数组，一共numclass[attributes]*attributes个数组，f4表示第f3个数组中对应的final class、attribute的class
				//std::cout << f3 << " " << f4 << " ";
			}
		}
	}
	...
	{	
		...
		{	
			...
			{	
				//如果第t个属性在final为d的情况下，值为o的总个数为零，就将该属性的所有属性值的和加1；
			...
			//在final为d的情况下，计算每个属性的属性值总数占record（final为d）的比例,P(ai=x|Ci)
	...
	for( int tt=0 ; tt< combinations ; tt++)
	{
		...
		{	
			...
			{
				//如果在final为dd时，combinations为tt时，attribute[i]和attributes[j]之间的某个相对应的值为0
				//就将这个关系中的所有对应值加一
		...
		for(int w1=0 ; w1 < numclass[attributes] ; w1++)
		{
			...
			{
				//在final为w1的情况下，计算每个属性之间的相关赋值占record的比例，P(ai=x and aj=y and Ci)
	...
	for( int ttt=0 ; ttt< combinations ; ttt++)
	{
		...
		{	
			...
			for(int w2=0 ; w2 < numclass[ rank[ttt][0] ]*numclass[ rank[ttt][1] ]  ; w2++ )
				//在final为w1的情况下，计算每个属性之间的相关赋值占record（final为d）的比例，P(ai=x and aj=y | Ci)
	...
	for(int combi=0 ; combi < combinations ; combi ++ )
	{
		...
		//如果属性mmm.value1和属性mmm.value2之间值相等且都不为零
		if( groups[mmm.value1] != 0 && groups[mmm.value2] != 0 && groups[mmm.value1] == groups[mmm.value2] )
		{}
		//如果属性mmm.value1和属性mmm.value2之间值相等且属性mmm.value1为零
		...
		//如果属性mmm.value2不为零且属性mmm.value1为零
		...
		//如果属性mmm.value1不为零且属性mmm.value2为零
		else if( groups[mmm.value1] != 0 && groups[mmm.value2] == 0 )
		{
	...
	for(int redo=0 ; redo<attributes ; redo++)
	{
		...
		//min为transfer中的最小值，point为transfer中的最小值所代表的属性
